...
- **`server/mod.rs`** — the realtime server: loads the `Recognizer` once, shares it as `Arc<Mutex<Recognizer>>` (inference runs via `spawn_blocking`), and serves the Leptos SSR routes + `/ws` on axum. `Engine::infer` wraps `Recognizer::transcribe_once`. The optional `--password` gate is **Basic auth on the page routes only** — `/ws` must stay outside that layer. Browsers don't attach cached Basic credentials to a WebSocket handshake, so a 401 there pops a second credential prompt that can never succeed. Instead an authenticated page response mints the `Auth` token as a session cookie (which the browser *does* send on the same-origin handshake) and `/ws` checks that, answering with a bare 401 (no `WWW-Authenticate`) so failures stay silent.
